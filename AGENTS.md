...
This repository contains a Python-based GLM/OpenAI-compatible API load testing suite. The main implementation is `glm_tpm_test.py`, which handles prompt generation, async load workers, retry logic, metrics aggregation, and report generation. Quick entry points live in `quick_test.py` and `quick_extreme_test.py`. API behavior checks are in `test_glm_features.py`. Generated outputs are written to `results/` as JSON, JSONL, Markdown, HTML, and CSV files. Older replaced scripts are kept in `archive/`; avoid editing them unless restoring historical behavior. Project documentation is in `CLAUDE.md`, `PROJECT_STRUCTURE.md`, `CHANGELOG.md`, and `压测脚本增强实施计划.md`.
...
```bash
python glm_tpm_test.py --base-url https://api.example.com/v1 --api-key $API_KEY --model glm-5.1 --concurrency 200 --duration-sec 300 --input-tokens 10000 --enable-stream
```

Run matrix testing across multiple input sizes and concurrency levels:

```bash
python glm_tpm_test.py --matrix-mode --input-tokens-list 1000,10000,100000 --concurrency-list 50,100,200 --matrix-duration-sec 60 --enable-stream
```
