...
当前项目已经具备命令行压测能力，核心脚本为 `glm_tpm_test.py`。本方案目标是将它改造为前后端一体、易部署的 Web 应用，让非命令行用户也能完成 LLM API 性能测试、查看实时进度、管理历史记录并导出报告。
...
- CLI 能力继续保留，Web 改造不能破坏 `python glm_tpm_test.py ...` 的使用方式。
...
2. 给 `LoadTester` 增加可选参数：`progress_callback`、`stop_event`、`log_callback`。
3. 后端通过 `WebTestRunner` 将 Web 配置转换为 `argparse.Namespace`，并调用现有运行和报告函数。
...
如果短期不想修改 `LoadTester` 构造函数，可以先用轮询方式读取 `tester.results` 推送进度。但长期推荐回调钩子，因为日志、停止任务和矩阵进度都更清晰。
...
- `python glm_tpm_test.py ...` 原 CLI 命令仍可运行。
- `python glm_tpm_test.py --help` 能看到 `--api-protocol {openai,anthropic,gemini}`。
