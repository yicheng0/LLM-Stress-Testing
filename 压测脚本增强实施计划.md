...
现有压测脚本 `glm_tpm_test.py` 存在以下关键缺陷：
...
**目标文件**：`D:\glm-test\glm_tpm_test.py`
...
| 功能 | 文件 | 行号范围 | 说明 |
|------|------|---------|------|
| 数据结构 | glm_tpm_test.py | 60-73 | RequestResult 定义 |
| 命令行参数 | glm_tpm_test.py | 511-530 | parse_args() |
| Payload 构建 | glm_tpm_test.py | 145-170 | build_payload() |
| 请求发送 | glm_tpm_test.py | 172-303 | send_one() |
| 统计汇总 | glm_tpm_test.py | 366-413 | summarize() |
| 报告生成 | glm_tpm_test.py | 416-508 | render_markdown_report() |
| 主流程 | glm_tpm_test.py | 539-568 | async_main() |
